...
export const PainSection: React.FC = () => {
  ...
  const [modalVideo, setModalVideo] = useState<{ id: string; title: string; category?: string } | null>(null);
  ...
  return (
    <section id="pain-points" className="py-24 px-4 bg-[#09090b] border-y border-zinc-800/80 text-white relative">
          ...
          {painPoints.tabs.map((tab, idx) => {
            ...
            return (
              <button
                ...
                className={`p-4 sm:p-5 rounded-2xl text-left transition-all duration-200 cursor-pointer border flex flex-col justify-between ${
                  isActive
                    ? 'bg-amber-500/20 border-amber-500 text-white shadow-xl ring-2 ring-amber-500/40'
                    : 'bg-zinc-900/90 border-zinc-700/80 text-zinc-300 hover:border-zinc-500 hover:text-white'
                }`}
              >
                <div className="flex items-center gap-2.5 mb-2">
                  <span className={`w-7 h-7 rounded-full text-xs sm:text-sm font-mono font-bold flex items-center justify-center shrink-0 ${
                    isActive ? 'bg-amber-400 text-zinc-950 font-black' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'
                  }`}>
                    {String(idx + 1).padStart(2, '0')}
                  </span>
                  <div className={`text-base sm:text-lg font-sans font-bold leading-snug ${isActive ? 'text-amber-300' : 'text-white'}`}>
                    {tab.title}
                  </div>
                </div>
                <div className={`text-sm font-sans leading-relaxed mt-1.5 ${isActive ? 'text-amber-100/90 font-medium' : 'text-zinc-300'}`}>
                  {tab.subtitle}
                </div>
              </button>
            );
        ...
        <div className="p-6 sm:p-10 rounded-3xl border border-zinc-700/80 bg-[#121216] shadow-2xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-7 flex flex-col justify-between">
              <div>
                <div className="inline-flex items-center gap-2 text-xs sm:text-sm font-mono font-bold text-amber-400 uppercase tracking-wider mb-2">
                  <AlertCircle className="w-4 h-4" />
                  <span>Điểm Nghẽn & Giải Pháp Thực Chiến</span>
                </div>
                <h3 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-3 leading-snug">
                  {currentTab.title}
                </h3>
                <p className="font-sans text-base sm:text-lg text-zinc-300 mb-6 leading-relaxed">
                  {currentTab.subtitle}
                </p>
                ...
                <div className="space-y-4 my-6">
                  {currentTab.points.map((point, idx) => (
                    <div key={idx} className="flex items-start gap-3 text-base sm:text-lg text-zinc-100 font-sans">
                ...
                {/* Outcome Box */}
                <div className="p-4 sm:p-5 rounded-2xl border border-emerald-500/40 bg-emerald-950/30 text-emerald-100 text-base sm:text-lg font-sans mb-6 leading-relaxed">
                  ✨ <strong>Giải pháp thực chiến:</strong> {currentTab.outcome.replace(/^Giải pháp:\s*/, '')}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-zinc-800">
                {currentTab.cards.map((c, idx) => (
                  <div key={idx} className="p-4 sm:p-5 rounded-xl border border-zinc-700 bg-zinc-900/90">
                    <div className="font-mono text-sm sm:text-base font-bold text-amber-300 mb-1.5">{c.title}</div>
                    <div className="text-sm text-zinc-200 leading-relaxed font-sans">{c.desc}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-5 flex flex-col items-center">
              {activeTab === 'tab-1' ? (
                ...
                <div className="w-full max-w-[380px] space-y-4">
                  ...
                  <div
                    onClick={() => setModalVideo({ id: activeBrollVideo.videoId, title: activeBrollVideo.title, category: 'Kho B-Roll Bank Xử Lý' })}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-[11px] font-sans font-bold text-zinc-200 truncate w-full leading-tight">
                            {bv.title.replace(' & B-Roll Đè Hình', '').replace('B-Roll ', '')}
                          </div>
                        </button>
                      );
                    })}
                ...
                <div className="w-full max-w-[380px] space-y-4">
                  ...
                  <div
                    onClick={() => setModalVideo({ id: activeScriptVideo.videoId, title: activeScriptVideo.title, category: 'Quy Trình Kịch Bản & Voice-Over' })}
                    className="relative rounded-3xl overflow-hidden border-2 border-amber-500 bg-zinc-950 shadow-2xl group cursor-pointer aspect-[9/16] flex items-center justify-center ring-2 ring-amber-500/30"
                  >
                    <img
                    ...
                    <div className="absolute bottom-0 inset-x-0 p-4 text-left">
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-amber-500 text-zinc-950 font-mono text-[11px] font-bold uppercase mb-1.5 shadow-sm">
                        <Video className="w-3.5 h-3.5" />
                        <span>KỊCH BẢN THỰC CHIẾN 0{selectedScriptIndex + 1}</span>
                      </div>
                      <h4 className="font-sans font-bold text-base text-white leading-snug line-clamp-1">
                    ...
                    {painPoints.scriptVideos.map((sv, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="font-mono text-[10px] font-bold text-amber-300">0{idx + 1}</div>
                          <div className="text-[11px] font-sans font-bold text-zinc-200 truncate w-full leading-tight">
                            {sv.title.replace('Kịch Bản ', '').replace('Dựng Ghép ', '')}
                          </div>
                        </button>
                      );
                    })}
      ...
      {modalVideo && (
        ...
        >
          <div
            className="relative w-full max-w-sm sm:max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl"
