    ...
    <section id="growth" className="py-24 px-4 bg-[#0c0d10] border-y border-zinc-800/80 relative">
      <div className="max-w-6xl mx-auto">
        <div className="text-center max-w-5xl mx-auto mb-16">
          ...
          <p className="text-zinc-400 text-base md:text-lg leading-relaxed [text-wrap:balance] max-w-3xl mx-auto">
            {chart.description}
          </p>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-6 text-xs sm:text-sm font-mono">
              <div className="flex items-center gap-2 text-emerald-400">
                ...
                <span className="font-bold">Video Marketing Thực Chiến (Tăng trưởng X10)</span>
              </div>
              <div className="flex items-center gap-2 text-orange-400">
                <div className="w-3 h-3 rounded-full bg-orange-500/80" />
                <span className="font-semibold">Video Tự Phát (Bản Năng)</span>
              </div>
            </div>
              ...
              <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible">
                ...
                {pointsMarketing.map((p, i) => (
                  <g key={`m-${i}`} className="group">
                    ...
                    <text x={p.x} y={p.y - 12} fill="#34d399" fontSize="13" fontWeight="bold" textAnchor="middle" fontFamily="monospace">
                      {p.val}%
                    </text>
                    <text x={p.x} y={height - padding + 22} fill="#e4e4e7" fontSize="13" fontWeight="bold" textAnchor="middle" fontFamily="monospace">
                      {p.month}
                    </text>
                  </g>
                ))}
