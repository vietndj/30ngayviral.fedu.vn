...
export function generateActivationEmailHtml({
  ...
  const supportZalo = courseConfig.instructor.zalo || "https://zalo.me/0934688632";
  const instructorPhone = courseConfig.instructor.phone || "0934.688.632";
...
<head>
  ...
  <title>Xác nhận học phí & Link vào lớp học</title>
  <style>
    @font-face {
      font-family: 'Acta';
          ...
          <tr>
            <td style="padding: 34px 28px 20px 28px;">
              ...
              <p style="margin: 0; font-size: 15.5px; line-height: 1.7; color: #475569;">
                Khoản học phí <strong style="color: #0f172a; font-weight: 700;">${courseConfig.price}đ</strong> của bạn đã được xác nhận thành công (Mã GD: <span style="font-family: monospace; color: #64748b;">${transactionId}</span>).
              </p>
            </td>
          </tr>
...
export async function sendCourseActivationEmail(params: SendActivationEmailParams) {
  ...
  const fromAddress = process.env.RESEND_FROM_EMAIL || "Lớp 30 ngày làm nội dung viral <viet@fedu.vn>";
  ...
  try {
    ...
    console.log(`[Resend] Successfully sent activation email to ${params.email}, id: ${result.data?.id}`);
